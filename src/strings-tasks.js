...
function removeFirstOccurrences(str, value) {
  ...
  const indexLast = value.length + indexFirst;
  return str.slice(0, indexFirst) + str.slice(indexLast);
}
...
function countVowels(str) {
  ...
  const vowelsCount = str.split('').reduce((acc, el) => {
...
function isPalindrome(str) {
  ...
  const palindrome = [...startStr].reverse();
  return startStr.join('') === palindrome.join('');
}
...
function reverseWords(str) {
  const arr = str.split(' ');
  const reverse = arr.reduce((acc, el) => {
...
function invertCase(str) {
  const arr = str.split('');
  const invert = arr.reduce((acc, el) => {
    acc.push(el === el.toLowerCase() ? el.toUpperCase() : el.toLowerCase());
    return acc;
  }, []);
  return invert.join('');
}
...
function extractNameFromTemplate(value) {
  ...
  const str = `${arr[1]} ${arr[2]}`.slice(0, -1);
  return str;
}
...
function encodeToRot13(str) {
  ...
  const resStr = [];
  arr.forEach((el) => {
    return input.includes(el)
      ? resStr.push(output[input.indexOf(el)])
      : resStr.push(el);
  });
  return resStr.join('');
}
